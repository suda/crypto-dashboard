<script>
  ...
  let chartElement, chart, candlestickSerie, volumeSerie, pricesData, volumeData;
  let change = 0;
  let lastPrice = 0;
  let totalVolume = 0;
  ...
  const getData = async () => {
    const data = await cc[`histo${timeframe}`](fsym, tsym);
...
<div id="chart-container">
  <div class="flex justify-between items-center h-12 pb-3">
    <div>
      <div class="text-xl md:text-sm text-gray-800 font-bold leading-tight">{fsym}-{tsym}</div>
      <div class="text-gray-400 md:text-xs">Market</div>
    </div>
    <div>
      <div class="text-xl md:text-sm text-gray-800 font-bold leading-tight">{lastPrice.toLocaleString()} {tsym}</div>
      <div class="text-gray-400 md:text-xs">Last trade price</div>
    </div>
    <div>
      <div class="text-xl md:text-sm text-gray-800 font-bold leading-tight {change > 0 ? 'text-green-500' : 'text-red-500'}">{change.toFixed(2)}%</div>
      <div class="text-gray-400 md:text-xs">24h price</div>
    </div>
    <div>
      <div class="text-xl md:text-sm text-gray-800 font-bold leading-tight">{totalVolume.toLocaleString(undefined, { minimumFractionDigits: 2 })} {fsym}</div>
      <div class="text-gray-400 md:text-xs">24h volume</div>
    </div>
  </div>
  <div id="chart" bind:this={chartElement} />
  {#if donationAddresses[fsym]}
  <div class="bg-purple-400 leading-none text-white rounded-lg p-2 shadow text-xs w-full">
